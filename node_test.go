...
func HostGossip(host string, t *testing.T, f func(*Config)) *Gossip {
	t.Helper()
	...
	c.Name = host
	c.BindAddr = host
	c.BindPort = bindPort // choose a free port
	...
	m, err := newGossip(c)
	if err != nil {
		t.Fatalf("failed to get Gossip: %s", err)
	}
	return m
}

func TestGossip_Probe(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()

	m1 := HostGossip(addr1.String(), t, func(c *Config) {
		c.ProbeTimeout = time.Millisecond
		c.ProbeInterval = 10 * time.Millisecond
	})
	defer m1.Shutdown()

	m2 := HostGossip(addr2.String(), t, func(c *Config) {
	})
	defer m2.Shutdown()
...
func TestGossip_ProbeNode_Suspect(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
	...
	defer m2.Shutdown()
	m3 := HostGossip(addr3.String(), t, func(c *Config) {
	})
	defer m3.Shutdown()
...
func TestGossip_ProbeNode_Suspect_Dogpile(t *testing.T) {
	cases := []struct {
		name          string
		numPeers      int
		confirmations int
	...
	for i, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			// Create the main Gossip under test.
			addr := getBindAddr()

			m := HostGossip(addr.String(), t, func(c *Config) {
				c.ProbeTimeout = time.Millisecond
			...
			for j := 0; j < c.numPeers-1; j++ {
				...
				peer := HostGossip(peerAddr.String(), t, func(c *Config) {
				})
				defer peer.Shutdown()
			...
			// to pings, but tell the Gossip it's alive.
...
/*
func TestGossip_ProbeNode_FallbackTCP(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
	...
	m4 := HostGossip(addr4.String(), t, func(c *Config) {
		c.BindPort = bindPort
	})
	defer m4.Shutdown()
...
func TestGossip_ProbeNode_FallbackTCP_Disabled(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
...
func TestGossip_ProbeNode_FallbackTCP_OldProtocol(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
...
func TestGossip_ProbeNode_Awareness_Degraded(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
...
func TestGossip_ProbeNode_Wrong_VSN(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
...
func TestGossip_ProbeNode_Awareness_Improved(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	ip1 := []byte(addr1)
	ip2 := []byte(addr2)
...
func TestGossip_ProbeNode_Awareness_MissedNack(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
...
func TestGossip_ProbeNode_Awareness_OldProtocol(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	addr3 := getBindAddr()
	addr4 := getBindAddr()
...
func TestGossip_ProbeNode_Buddy(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	ip1 := []byte(addr1)
	ip2 := []byte(addr2)
...
func TestGossip_ProbeNode(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	ip1 := []byte(addr1)
	ip2 := []byte(addr2)
...
func TestGossip_Ping(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	ip1 := []byte(addr1)
	ip2 := []byte(addr2)
...
func TestGossip_ResetNodes(t *testing.T) {
	m := GetGossip(t, func(c *Config) {
...
func TestGossip_NextSeq(t *testing.T) {
	m := &Gossip{}
	if m.nextSeqNo() != 1 {
...
func ackHandlerExists(t *testing.T, m *Gossip, idx uint32) bool {
	t.Helper()
...
func TestGossip_setProbeChannels(t *testing.T) {
	m := &Gossip{ackHandlers: make(map[uint32]*ackHandler)}
...
func TestGossip_setAckHandler(t *testing.T) {
	m := &Gossip{ackHandlers: make(map[uint32]*ackHandler)}
...
func TestGossip_invokeAckHandler(t *testing.T) {
	m := &Gossip{ackHandlers: make(map[uint32]*ackHandler)}
...
func TestGossip_invokeAckHandler_Channel_Ack(t *testing.T) {
	m := &Gossip{ackHandlers: make(map[uint32]*ackHandler)}
...
func TestGossip_invokeAckHandler_Channel_Nack(t *testing.T) {
	m := &Gossip{ackHandlers: make(map[uint32]*ackHandler)}
...
func TestGossip_AliveNode_NewNode(t *testing.T) {
...
func TestGossip_AliveNode_SuspectNode(t *testing.T) {
	ch := make(chan NodeEvent, 1)
...
func TestGossip_AliveNode_Idempotent(t *testing.T) {
	ch := make(chan NodeEvent, 1)
	ted := &toggledEventDelegate{
		real: &ChannelEventDelegate{ch},
	}
...
func TestGossip_AliveNode_ChangeMeta(t *testing.T) {
	ch := make(chan NodeEvent, 1)
	ted := &toggledEventDelegate{
		real: &ChannelEventDelegate{ch},
	}
...
func TestGossip_AliveNode_Refute(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_AliveNode_Conflict(t *testing.T) {
	m := GetGossip(t, func(c *Config) {
...
func TestGossip_SuspectNode_NoNode(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_SuspectNode(t *testing.T) {
	m := GetGossip(t, func(c *Config) {
...
func TestGossip_SuspectNode_DoubleSuspect(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_SuspectNode_OldSuspect(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_SuspectNode_Refute(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_DeadNode_NoNode(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_DeadNode(t *testing.T) {
	ch := make(chan NodeEvent, 1)
...
func TestGossip_DeadNode_Double(t *testing.T) {
	ch := make(chan NodeEvent, 1)
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_DeadNode_OldDead(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_DeadNode_AliveReplay(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_DeadNode_Refute(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_MergeState(t *testing.T) {
	m := GetGossip(t, nil)
	defer m.Shutdown()
...
func TestGossip_Gossip(t *testing.T) {
	...
	m3 := HostGossip(addr2.String(), t, func(c *Config) {
	})
	defer m3.Shutdown()
...
func TestGossip_GossipToDead(t *testing.T) {
...
func TestGossip_PushPull(t *testing.T) {
	addr1 := getBindAddr()
	addr2 := getBindAddr()
	ip1 := []byte(addr1)
	ip2 := []byte(addr2)
