...
func (t *NetTransport) tcpListen(tcpLn *net.TCPListener) {
	...
	for {
		...
		if err != nil {
			...
			t.logger.Printf("[ERR] gossip: Error accepting TCP connection: %v", err)
...
func (t *NetTransport) udpListen(udpLn *net.UDPConn) {
	...
	for {
		...
		if err != nil {
			...
			t.logger.Printf("[ERR] gossip: Error reading UDP packet: %v", err)
			continue
		}
		...
		// proper message.
		if n < 1 {
			t.logger.Printf("[ERR] gossip: UDP packet too short (%d bytes) %s",
				len(buf), LogAddress(addr))
			continue
		}

		// Ingest the packet.
		metrics.IncrCounter([]string{"gossip", "udp", "received"}, float32(n))
		t.packetCh <- &Packet{
			Buf:       buf[:n],
			From:      addr,
			Timestamp: ts,
