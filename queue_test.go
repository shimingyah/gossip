...
func TestTransmitLimited_Queue(t *testing.T) {
	...
	q.QueueBroadcast(&gossipBroadcast{"test", nil, nil})
	q.QueueBroadcast(&gossipBroadcast{"foo", nil, nil})
	q.QueueBroadcast(&gossipBroadcast{"bar", nil, nil})

	if q.NumQueued() != 3 {
		t.Fatalf("bad len")
	}
	dump := q.orderedView(true)
	if dump[0].b.(*gossipBroadcast).node != "test" {
		t.Fatalf("missing test")
	}
	if dump[1].b.(*gossipBroadcast).node != "foo" {
		t.Fatalf("missing foo")
	}
	if dump[2].b.(*gossipBroadcast).node != "bar" {
	...
	dump = q.orderedView(true)
	if dump[0].b.(*gossipBroadcast).node != "foo" {
		t.Fatalf("missing foo")
	}
	if dump[1].b.(*gossipBroadcast).node != "bar" {
		t.Fatalf("missing bar")
	}
	if dump[2].b.(*gossipBroadcast).node != "test" {
		t.Fatalf("missing test")
	}
}
...
func TestTransmitLimited_GetBroadcasts(t *testing.T) {
	...
	q.QueueBroadcast(&gossipBroadcast{"test", []byte("1. this is a test."), nil})
	q.QueueBroadcast(&gossipBroadcast{"foo", []byte("2. this is a test."), nil})
	q.QueueBroadcast(&gossipBroadcast{"bar", []byte("3. this is a test."), nil})
	q.QueueBroadcast(&gossipBroadcast{"baz", []byte("4. this is a test."), nil})
...
func TestTransmitLimited_Prune(t *testing.T) {
	...
	// 18 bytes per message
	q.QueueBroadcast(&gossipBroadcast{"test", []byte("1. this is a test."), ch1})
	q.QueueBroadcast(&gossipBroadcast{"foo", []byte("2. this is a test."), ch2})
	q.QueueBroadcast(&gossipBroadcast{"bar", []byte("3. this is a test."), nil})
	q.QueueBroadcast(&gossipBroadcast{"baz", []byte("4. this is a test."), nil})
	...
	if dump[0].b.(*gossipBroadcast).node != "bar" {
		t.Fatalf("missing bar")
	}
	if dump[1].b.(*gossipBroadcast).node != "baz" {
...
func TestTransmitLimited_ordering(t *testing.T) {
	...
	insert := func(name string, transmits int) {
		q.queueBroadcast(&gossipBroadcast{name, []byte(name), make(chan struct{})}, transmits)
	}
	...
	if dump[0].transmits != 10 {
		t.Fatalf("bad val %v, %d", dump[0].b.(*gossipBroadcast).node, dump[0].transmits)
	}
	if dump[1].transmits != 7 {
		t.Fatalf("bad val %v, %d", dump[7].b.(*gossipBroadcast).node, dump[7].transmits)
	}
	if dump[2].transmits != 4 {
		t.Fatalf("bad val %v, %d", dump[2].b.(*gossipBroadcast).node, dump[2].transmits)
	}
	if dump[3].transmits != 3 {
		t.Fatalf("bad val %v, %d", dump[3].b.(*gossipBroadcast).node, dump[3].transmits)
	}
	if dump[4].transmits != 0 {
		t.Fatalf("bad val %v, %d", dump[4].b.(*gossipBroadcast).node, dump[4].transmits)
	}
}
